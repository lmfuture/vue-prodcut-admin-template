<template>
	<div>
		<div class="common-title">投保须知</div>
		<el-upload
	    ...
	    <div style="text-align: center;padding-top: 30px;margin-bottom: 50px;">
			<el-button v-show='productOption=="xinzeng"' type="primary" @click="changeActiveName('second')">上一步</el-button>
			<el-button type="success" @click="changeActiveName()">保 存</el-button>
		    <el-button v-show='productOption=="xinzeng"' type="primary" @click="changeActiveName('fourth')">下一步</el-button>
		</div>
	</div>
</template>

<script>
	import { addInsuranceNotes , getProductById } from '@/api/product'
	...
	export default {
		...
		created(){
			...
			getProductById({id:this.id}).then(res => {
				if(res.result.insAttention){
					this.content = res.result.insAttention;
				}
			})
		},
		computed:{
		...
		methods: {
	      ...
	      changeActiveName(step) {
	      	if(!step){
		      	...
		      	const addData = {
		      		id:this.id,
		      		insAttention:this.content
		      	}
		      	addInsuranceNotes(Qs.stringify(addData)).then(res => {
		      		if(res.code == '100100'){
		      			this.$message({
				          message: '保存成功',
				          type: 'success'
